...
def plot_logs(filename, title):
    x = []
    y = []
    # read the log files first.
    with open(os.path.join("logs", filename), "r") as logfile:
        for line in logfile:
            match = re.search('^ep: (\d+)[^+-]*([+-]?\d+\.\d+)$', line)
            if match is not None:
                x.append(int(match.groups()[0]))
                y.append(float(match.groups()[1]))
